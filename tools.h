...
void correctIndices(int n, condition *list, int total) {
  for (int i = 0; i < n; i++) list[i].setNode1(list[i].getNode1() + total);
}
...
void fusionDirichlet(int n1, condition *list1, int n2, condition *list2, int n3,
                     condition *list3, int n4, condition *list4,
                     condition *list) {
  int index = 0;
...
void leerMallaYCondiciones(mesh &m, char *filename) {
  ...
  fusionDirichlet(ndirich_u, dirichlet_u, ndirich_v, dirichlet_v, ndirich_w,
                  dirichlet_w, ndirich_p, dirichlet_p, m.getDirichlet());

  correctConditions(ndirich_u + ndirich_v + ndirich_w + ndirich_p,
                    m.getDirichlet(), m.getDirichletIndices());
}
...
void writeResults(mesh m, Vector T, char *filename) {
  ...
  file << "Result \"Velocity\" \"Load Case 1\" 1 Vector "
  ...
  file << "\nResult \"Pressure\" \"Load Case 1\" 1 Scalar "
          "OnNodes\nComponentNames \"p\"\nValues\n";
