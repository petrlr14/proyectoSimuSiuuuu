...
void copyMatrix(Matrix A, Matrix &copy) {
  ...
  for (int i = 0; i < A.size(); i++)
    for (int j = 0; j < A.at(0).size(); j++) copy.at(i).at(j) = A.at(i).at(j);
}

float calculateMember(int i, int j, int r, Matrix A, Matrix B) {
  float member = 0;
  for (int k = 0; k < r; k++) member += A.at(i).at(k) * B.at(k).at(j);
  return member;
}
...
Matrix productMatrixMatrix(Matrix A, Matrix B, int n, int r, int m) {
  ...
  zeroes(R, n, m);
  for (int i = 0; i < n; i++)
    for (int j = 0; j < m; j++) R.at(i).at(j) = calculateMember(i, j, r, A, B);
...
Matrix sumMatrix(Matrix A, Matrix B, int n, int m) {
  ...
  zeroes(R, n, m);
  for (int i = 0; i < n; i++)
    for (int j = 0; j < m; j++) R.at(i).at(j) = A.at(i).at(j) + B.at(i).at(j);
  return R;
}
...
void productRealVector(float real, Vector V, Vector &R) {
  zeroes(R, V.size());
  for (int i = 0; i < V.size(); i++) R.at(i) = real * V.at(i);
}
...
void getMinor(Matrix &M, int i, int j) {
  M.erase(M.begin() + i);
  for (int i = 0; i < M.size(); i++) M.at(i).erase(M.at(i).begin() + j);
}
...
void transpose(Matrix M, Matrix &T) {
  ...
  for (int i = 0; i < M.size(); i++)
    for (int j = 0; j < M.at(0).size(); j++) T.at(j).at(i) = M.at(i).at(j);
}
